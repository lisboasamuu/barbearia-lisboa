...
describe("canonical route shortcuts", () => {
  it("declares shortcuts for booking and administration", () => {
    expect(redirects.redirects).toEqual(expect.arrayContaining([
      { source: "/agendar", destination: "/barbearia-lisboa/agendar", permanent: false },
      { source: "/agenda", destination: "/barbearia-lisboa/agenda", permanent: false },
    ]));
  });
});
